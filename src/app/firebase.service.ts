...
export class FirebaseService {
  ...
  listDemirbasByID(personelId) {
    const demirbaslar = collection(
      this.fireStore,
      `personel/${personelId}/demirbaslar`
    );
  ...
  updatePersonel(newPersonel: personeller) {
    const currentPersonel = doc(this.fireStore, `personel/${newPersonel.id}`);
    return updateDoc(currentPersonel, {
      ad: newPersonel.ad,
      soyad: newPersonel.soyad,
      sicilNo: newPersonel.sicilNo,
    });
  }

  deletePersonelById(personelId){

    const personel = doc(this.fireStore, `personel/${personelId}`);
    return deleteDoc(personel);
  }
}
